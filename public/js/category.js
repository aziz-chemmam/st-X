$(document).ready(function() {
    function fetshCategory(response) {
        ...
        $.each(response, function(index, row){
            ...
            <div class="w-96 px-8 py-4 bg-white rounded-lg text-black shadow-md dark:bg-orange-50">
            <div class="flex items-center justify-between">
                <span class="text-sm font-light text-gray-600 dark:text-gray-400">${row.created_at}</span>
                <h1 class="px-3 py-1 text-sm font-bold text-gray-100 transition-colors duration-300 transform bg-orange-600 rounded cursor-pointer  hover:bg-orange-700">Category</h1>
            </div>

            <div class="mt-2">
                <p class="text-xl font-bold text-black">${row.categoryName}</p>
                <p class="mt-2 text-gray-600">${row.categoryDesc}</p>
            </div>
